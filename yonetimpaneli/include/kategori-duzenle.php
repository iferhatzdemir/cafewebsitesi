    ...
    <div class="container-fluid">
      ...
      if($_POST)
      {
       if(!empty($_POST["kategori"]) && !empty($_POST["baslik"]) && !empty($_POST["anahtar"]) && !empty($_POST["description"]) && !empty($_POST["sirano"]))
       {
         ...
         $tr_kategori=$VT->seflink($kategori);//ömer ekledi
         ...
         if(!empty($_FILES["resim"]["name"]))
         {
           ...
           if($yukle!=false)
           {
             $ekle=$VT->SorguCalistir("UPDATE kategoriler","SET baslik=?, seflink=?,tablo=?,resim=?, anahtar=?, description=?, durum=?, sirano=?, tarih=? WHERE ID=?",array($baslik,$seflink,$tr_kategori/*ömer ekledi*/,$yukle,$anahtar,$description,1,$sirano,date("Y-m-d"),$kontrol[0]["ID"]),1);
           }
           else
           {
            ?>
        ...
        {
         $ekle=$VT->SorguCalistir("UPDATE kategoriler ","SET baslik=?, seflink=?, tablo=?,  anahtar=?, description=?, durum=?, sirano=?, tarih=? WHERE ID=?",array($baslik,$seflink,$tr_kategori,$anahtar,$description,1,$sirano,date("Y-m-d"),$kontrol[0]["ID"]),1);
       }
