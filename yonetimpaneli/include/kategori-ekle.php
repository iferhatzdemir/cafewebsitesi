      ...
      if($_POST)
      {
       if(!empty($_POST["kategori"]) && !empty($_POST["baslik"]) && !empty($_POST["anahtar"]) && !empty($_POST["description"]) && !empty($_POST["sirano"]))
       {
         ...
         if(!empty($_FILES["resim"]["name"]))
         {
           ...
           if($yukle!=false)
           {
             $ekle=$VT->SorguCalistir("INSERT INTO kategoriler","SET baslik=?, seflink=?,tablo=?,resim=?, anahtar=?, description=?, durum=?, sirano=?, tarih=?",array($baslik,$seflink,$tr_kategori/*ömer ekledi*/,$yukle,$anahtar,$description,1,$sirano,date("Y-m-d")));
           }
           else
           {
            ?>
        ...
        {
         $ekle=$VT->SorguCalistir("INSERT INTO kategoriler ","SET baslik=?, seflink=?, tablo=?,  anahtar=?, description=?, durum=?, sirano=?, tarih=?",array($baslik,$seflink,$tr_kategori,$anahtar,$description,1,$sirano,date("Y-m-d")));
       }
